import java.util.ArrayList;

class Timbre {

    public static final int ANNEE_COURANTE = 2016;
    public static final int VALEUR_TIMBRE_DEFAUT = 1;
    public static final String PAYS_DEFAUT = "Suisse";
    public static final String CODE_DEFAUT = "lambda";

    public static final int BASE_1_EXEMPLAIRES = 100;
    public static final int BASE_2_EXEMPLAIRES = 1000;
    public static final double PRIX_BASE_1 = 600;
    public static final double PRIX_BASE_2 = 400;
    public static final double PRIX_BASE_3 = 50;

    /*******************************************
     * Completez le programme a partir d'ici.
     *******************************************/

    private String code;
    private int emissionYear;
    private String originCountry;
    private double facialValue;

    public Timbre()
    {
        this.code = CODE_DEFAUT;
        this.emissionYear = ANNEE_COURANTE;
        this.originCountry = PAYS_DEFAUT;
        this.facialValue = VALEUR_TIMBRE_DEFAUT;        
    }

    public Timbre(String code)
    {
        this.code = code;
        this.emissionYear = ANNEE_COURANTE;
        this.originCountry = PAYS_DEFAUT;
        this.facialValue = VALEUR_TIMBRE_DEFAUT;        
    }

    public Timbre(String code, int emissionYear)
    {
        this.code = code;
        this.emissionYear = emissionYear;
        this.originCountry = PAYS_DEFAUT;
        this.facialValue = VALEUR_TIMBRE_DEFAUT;        
    }

    public Timbre(String code, int emissionYear, String originCountry)
    {
        this.code = code;
        this.emissionYear = emissionYear;
        this.originCountry = originCountry;
        this.facialValue = VALEUR_TIMBRE_DEFAUT;        
    }

    public Timbre(String code, int emissionYear, String originCountry, double facialValue)
    {
        this.code = code;
        this.emissionYear = emissionYear;
        this.originCountry = originCountry;
        this.facialValue = facialValue;        
    }

    public String getCode()
    {
        return this.code;
    }

    public int getAnnee()
    {
        return this.emissionYear;
    }

    public String getPays()
    {
        return this.originCountry;
    }

    public double getValeurFaciale()
    {
        return this.facialValue;
    }

    public double vente()
    {
        int age = this.age();

        if (age < 5)
        {
            return this.facialValue;
        }
        else
        {
            return this.facialValue * age * 2.5;
        }
    }

    public int age()
    {
        return ANNEE_COURANTE - this.emissionYear;
    }

    public String toString()
    {
        return "Timbre de code "
            + this.code
            + " datant de "
            + this.emissionYear
            + " (provenance "
            + this.originCountry
            + ") ayant pour valeur faciale "
            + this.facialValue
            + " francs";
    }
}

class Rare extends Timbre {

    private int nbExamples;

    public Rare(int nbExamples)
    {
        super();
        this.nbExamples = nbExamples;        
    }

    public Rare(String code, int nbExamples)
    {
        super(code);
        this.nbExamples = nbExamples;        
    }

    public Rare(String code, int emissionYear, int nbExamples)
    {
        super(code, emissionYear);
        this.nbExamples = nbExamples;        
    }

    public Rare(String code, int emissionYear, String originCountry, int nbExamples)
    {
        super(code, emissionYear, originCountry);
        this.nbExamples = nbExamples;        
    }

    public Rare(String code, int emissionYear, String originCountry, double facialValue, int nbExamples)
    {
        super(code, emissionYear, originCountry, facialValue);
        this.nbExamples = nbExamples;        
    }

    public int getExemplaires()
    {
        return this.nbExamples;
    }

    public double vente()
    {
        double basePrice;

        if (this.nbExamples < BASE_1_EXEMPLAIRES) {
            basePrice = PRIX_BASE_1;
        } else if (this.nbExamples < BASE_2_EXEMPLAIRES) {
            basePrice = PRIX_BASE_2;
        } else {
            basePrice = PRIX_BASE_3;
        }

        return basePrice * (super.age() / 10.0);
    }

    public String toString()
    {
        return super.toString() + "\nNombre d'exemplaires -> " + this.nbExamples;
    }
}

class Commemoratif extends Timbre {

    public Commemoratif()
    {
        super();
    }

    public Commemoratif(String code)
    {
        super(code);
    }

    public Commemoratif(String code, int emissionYear)
    {
        super(code, emissionYear);
    }

    public Commemoratif(String code, int emissionYear, String originCountry)
    {
        super(code, emissionYear, originCountry);
    }

    public Commemoratif(String code, int emissionYear, String originCountry, double facialValue)
    {
        super(code, emissionYear, originCountry, facialValue);
    }

    public double vente()
    {
        return super.vente() * 2.0;
    }

    public String toString()
    {
        return super.toString() + "\nTimbre celebrant un evenement";
    }
}
	
/*******************************************
 * Ne rien modifier apres cette ligne.
 *******************************************/

class Philatelie {

    public static void main(String[] args) {

        // ordre des parametres: nom, annee d'emission, pays, valeur faciale,
        // nombre d'exemplaires
        Rare t1 = new Rare("Guarana-4574", 1960, "Mexique", 0.2, 98);

        // ordre des parametres: nom, annee d'emission, pays, valeur faciale
        Commemoratif t2 = new Commemoratif("700eme-501", 2002, "Suisse", 1.5);
        Timbre t3 = new Timbre("Setchuan-302", 2004, "Chine", 0.2);

        Rare t4 = new Rare("Yoddle-201", 1916, "Suisse", 0.8, 3);


        ArrayList<Timbre> collection = new ArrayList<Timbre>();

        collection.add(t1);
        collection.add(t2);
        collection.add(t3);
        collection.add(t4);

        for (Timbre timbre : collection) {
            System.out.println(timbre);
            System.out.println("Prix vente : " + timbre.vente() + " francs");
            System.out.println();
        }
    }
}

